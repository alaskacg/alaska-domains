import { Mail, TrendingUp, ShoppingCart, Loader2 } from "lucide-react";
import { useState } from "react";
import { DOMAIN_PRICES, redirectToCheckout } from "@/lib/stripe";
...
const DomainCard = ({ name, price, category, status = 'available' }: DomainCardProps) => {
  ...
  const hasStripeProduct = DOMAIN_PRICES[name.toLowerCase()] !== undefined;
  const isSold = status !== 'available';

  const handleBuyNow = async () => {
    setLoading(true);
    try {
      await redirectToCheckout(name);
    } catch (err) {
  ...
  return (
    <div className="glass-morphism rounded-2xl p-5 hover-lift border border-primary/20 group relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        {!isSold && hasStripeProduct ? (
          <div className="space-y-2">
            <button
              onClick={handleBuyNow}
              disabled={loading}
              className="w-full inline-flex items-center justify-center gap-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white px-4 py-2.5 rounded-xl font-sans text-sm font-semibold hover:scale-105 transition-transform shadow-lg group-hover:shadow-green-500/30 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {loading ? (
            ...
            <a 
              ...
              className="w-full inline-flex items-center justify-center gap-2 bg-transparent border border-primary/30 text-foreground px-4 py-2 rounded-xl font-sans text-xs font-medium hover:bg-primary/10 transition-colors"
            >
